...
//==========================================================================================
/* Runtime Validation */
#define DMA_IS_VALID_ADDR(addr) \
    (((uint32_t)(addr) >= 0x20000000UL) && ((uint32_t)(addr) < 0x2000C000UL))

#define DMA_IS_IN_POOL(addr) \
    (((uint32_t)(addr) >= (uint32_t)&__dma_pool_start__) && \
     ((uint32_t)(addr) <  (uint32_t)&__dma_pool_end__))

#define DMA_POOL_SIZE()     ((uint32_t)&__dma_pool_end__ - (uint32_t)&__dma_pool_start__)
#define DMA_POOL_START()    ((void*)&__dma_pool_start__)
#define DMA_POOL_END()      ((void*)&__dma_pool_end__)
//==========================================================================================

static inline bool hal_drv_dma_addr_is_safe(uint32_t addr, uint32_t len)
{
    return (addr >= 0x20000000U) &&
...
typedef struct {
    ...
    uint32_t m2m_src_addr;
    uint32_t m2m_dest_addr;
    uint32_t m2m_len;
    uint32_t trig;
    uint32_t buf_base_addr_a;
    uint32_t buf_base_addr_b;
} DMA_CH_BACKUP_T;

// Backup storage for all DMA channels
static DMA_CH_BACKUP_T dma_bkup_ch_regs[CHIP_DMA_CHANNEL_NUM];
...
{
    DMA_ASSERT_SAFE_RET(addr, len, HAL_DMA_ERR_UNSAFE_ADDR);
...
HAL_STATUS hal_drv_dma_p2m2p_trigger(uint8_t ch, uint32_t addr, uint32_t len)
{
    DMA_ASSERT_SAFE_RET(addr, len, HAL_DMA_ERR_UNSAFE_ADDR);
    uint32_t ch_ofs = DMA_CH_OFS(ch);    
    dma_set_channel_buf_er8130(ch_ofs, DMA_BUF_A, addr, len);
...
HAL_STATUS hal_drv_dma_p2m2p_config_and_trigger(uint8_t ch, uint8_t req_idx, uint32_t addr, uint32_t len)
{
    DMA_ASSERT_SAFE_RET(addr, len, HAL_DMA_ERR_UNSAFE_ADDR);
...
void hal_drv_dma_backup_channels(void)
{
    ...
    // Save the settings of all DMA channels before sleep
    for (int i = 0; i < CHIP_DMA_CHANNEL_NUM; i++) {
        ch_ofs = i * CH_ADDR_ALIGN;
        dma_bkup_ch_regs[i].config         = REG_R32(DMA_CH0_CONFIG_REG_ADDR + ch_ofs);
        dma_bkup_ch_regs[i].buf_word_size  = REG_R32(DMA_CH0_BUF_WORD_SIZE_REG_ADDR + ch_ofs);
        dma_bkup_ch_regs[i].m2m_src_addr   = REG_R32(DMA_CH0_M2M_SRC_ADDR_REG_ADDR + ch_ofs);
        dma_bkup_ch_regs[i].m2m_dest_addr  = REG_R32(DMA_CH0_M2M_DEST_ADDR_REG_ADDR + ch_ofs);
        dma_bkup_ch_regs[i].m2m_len        = REG_R32(DMA_CH0_M2M_LEN_REG_ADDR + ch_ofs);
        dma_bkup_ch_regs[i].trig           = REG_R32(DMA_CH0_TRIG_REG_ADDR + ch_ofs);
        dma_bkup_ch_regs[i].buf_base_addr_a = REG_R32(DMA_CH0_BUF_BASE_ADDR_A_REG_ADDR + ch_ofs);
        dma_bkup_ch_regs[i].buf_base_addr_b = REG_R32(DMA_CH0_BUF_BASE_ADDR_B_REG_ADDR + ch_ofs);
    }
}

void hal_drv_dma_restore_channels(void)
{
    // Restore the settings of all DMA channels after wakeup from sleep
    for (int i = 0; i < CHIP_DMA_CHANNEL_NUM; i++) {
        uint32_t ch_ofs = i * CH_ADDR_ALIGN;
        REG_W32(DMA_CH0_CONFIG_REG_ADDR + ch_ofs, dma_bkup_ch_regs[i].config);
        REG_W32(DMA_CH0_BUF_WORD_SIZE_REG_ADDR + ch_ofs, dma_bkup_ch_regs[i].buf_word_size);
        REG_W32(DMA_CH0_M2M_SRC_ADDR_REG_ADDR + ch_ofs, dma_bkup_ch_regs[i].m2m_src_addr);
        REG_W32(DMA_CH0_M2M_DEST_ADDR_REG_ADDR + ch_ofs, dma_bkup_ch_regs[i].m2m_dest_addr);
        REG_W32(DMA_CH0_M2M_LEN_REG_ADDR + ch_ofs, dma_bkup_ch_regs[i].m2m_len);
        REG_W32(DMA_CH0_TRIG_REG_ADDR + ch_ofs, dma_bkup_ch_regs[i].trig);
        REG_W32(DMA_CH0_BUF_BASE_ADDR_A_REG_ADDR + ch_ofs, dma_bkup_ch_regs[i].buf_base_addr_a);
        REG_W32(DMA_CH0_BUF_BASE_ADDR_B_REG_ADDR + ch_ofs, dma_bkup_ch_regs[i].buf_base_addr_b);
    }
}

HAL_STATUS hal_drv_dma_extsrc_m2m_config(uint8_t channel, uint8_t ext_src_type, DMA_EXT_DIR_TYPE trans_type, uint32_t data_addr, uint32_t data_size,
                                   uint32_t data_len)
{
    DMA_ASSERT_SAFE_RET(data_addr, data_len, HAL_DMA_ERR_UNSAFE_ADDR);
    uint32_t ch_ofs = DMA_CH_OFS(channel);
...
HAL_STATUS hal_drv_dma_m2m_config(uint8_t channel, uint32_t src_addr, uint32_t dst_addr, uint32_t data_size, uint32_t data_len)
{
    DMA_ASSERT_SAFE_RET(src_addr, data_len, HAL_DMA_ERR_UNSAFE_ADDR);
    DMA_ASSERT_SAFE_RET(dst_addr, data_len, HAL_DMA_ERR_UNSAFE_ADDR);
