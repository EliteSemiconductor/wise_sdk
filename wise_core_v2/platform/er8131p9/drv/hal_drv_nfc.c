...
void hal_drv_nfc_set_wakeup_config(uint8_t pwr_mode)
{
    switch(pwr_mode) {
    case PWR_MODE_SLEEP:
        // system enter sleep mode and wakeup by nfc
        hal_drv_pmu_set_wakeup_by_nfc(ENABLE);
        hal_drv_pmu_set_interrupt(PMU_INT_SRC_NFC);
        break;
    case PWR_MODE_SHUTDOWN:
        // system shutdown mode and wakeup by nfc
