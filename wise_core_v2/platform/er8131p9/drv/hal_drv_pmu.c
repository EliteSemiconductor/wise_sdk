...
{
    pmu_set_xip_clk_er8130(clk_fac);
}
...
{
    pmu_set_sram_pd_mode_er8130(sram_32_64);
}

void hal_drv_pmu_set_wakeup_by_nfc(uint8_t enable)
{
    pmu_set_wakeup_by_nfc_er8130(enable);
}

void hal_drv_pmu_enable_bod_reset(uint8_t enable)
{
    pmu_enable_bod_reset_er8130(enable);
}

uint32_t hal_drv_pmu_get_int_status(void)
{
    return pmu_get_int_status_er8130();
}
