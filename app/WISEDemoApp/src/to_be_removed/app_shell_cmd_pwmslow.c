...
#include <stdlib.h>
#include <string.h>
#include "app_shell_cmd_pwmslow.h"
...
static const struct shellCommand m_CmdList[] = {
    {"init", "Initialize PWMSLOW", _initCmd},
...
static int _configPeriodCmd(int argc, char **argv)
{
    if (argc < 8) {
        printf(
            "Usage: pwmslow config_period <gpio_index> <period> "
    ...
    pwmslow_conf.mode            = PWMSLOW_MODE_PERIOD;
    pwmslow_conf.period          = atoi(argv[3]);
    pwmslow_conf.active_period   = atoi(argv[4]);
    pwmslow_conf.center_align_en = atoi(argv[5]);
    pwmslow_conf.oneshot_en      = atoi(argv[6]);
    pwmslow_conf.oneshot_num     = atoi(argv[7]);

    if (wise_pwmslow_configure(gpio_index, &pwmslow_conf) == WISE_SUCCESS) {
        printf("PWMSLOW configured successfully with period control.\n");
        if (pwmslow_conf.oneshot_en == ENABLE) {
...
static int _configFrequencyCmd(int argc, char **argv)
{
    ...
    pwmslow_conf.mode          = PWMSLOW_MODE_FREQUENCY;
    pwmslow_conf.frequency_Hz  = atoi(argv[3]);
    pwmslow_conf.duty_percent  = atoi(argv[4]);
    pwmslow_conf.idle_status   = atoi(argv[5]);
    pwmslow_conf.low_active_en = atoi(argv[6]);
    pwmslow_conf.oneshot_en    = atoi(argv[7]);
    pwmslow_conf.oneshot_num   = atoi(argv[8]);

    if (wise_pwmslow_configure(gpio_index, &pwmslow_conf) == WISE_SUCCESS) {
        printf("PWMSLOW configured successfully with frequency control.\n");
        if (pwmslow_conf.oneshot_en == ENABLE) {
            wise_pwmslow_register_callback(PWMSLOW_EVENT_TRIGGER,
                                           my_pwmslow_callback, NULL);
            printf("PWMSLOW interrupt enabled and callback registered.\n");
        }
    } else {
        printf("PWMSLOW configuration failed.\n");
...
static int _demoCmd(int argc, char **argv)
{
    ...
    if (wise_pwmslow_configure(gpio_pin, &pwmslow_conf) == WISE_SUCCESS) {
        printf("PWMSLOW configured with frequency %dHz frequency duty percent "
               "%d%% and interrupt enabled\n",
               pwmslow_conf.frequency_Hz, pwmslow_conf.duty_percent);
        wise_pwmslow_register_callback(PWMSLOW_EVENT_TRIGGER,
                                       my_pwmslow_callback, NULL);
    } else {
