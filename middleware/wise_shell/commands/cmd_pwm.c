...
static int _pwmInitCmd(int argc, char **argv);
...
static const struct shellCommand m_CmdList[] = {
    {"init", "Initialize PWM", _pwmInitCmd},
...
{
    if (argc < 9) {
        printf("Usage: pwm config_period <channel> <gpio_index> <period> "
    ...
    pwm_conf.common.mode            = PWM_MODE_PERIOD;
    pwm_conf.u.period.period        = atoi(argv[4]);
    pwm_conf.u.period.active_period = atoi(argv[5]);
    pwm_conf.common.center_align_en = atoi(argv[6]);
    pwm_conf.common.oneshot_en      = atoi(argv[7]);
    pwm_conf.common.oneshot_num     = atoi(argv[8]);
...
static int _configFrequencyCmd(int argc, char **argv)
{
    ...
    pwm_conf.u.freq.frequency_Hz  = atoi(argv[4]);
    pwm_conf.u.freq.duty_percent  = atoi(argv[5]);
    pwm_conf.common.idle_status   = atoi(argv[6]);
    pwm_conf.common.low_active_en = atoi(argv[7]);
    pwm_conf.common.oneshot_en    = atoi(argv[8]);
    pwm_conf.common.oneshot_num   = atoi(argv[9]);
...
static int _startCmd(int argc, char **argv)
{
    ...
    printf("PWM started on channel mask: 0x%lx\n", channel_mask);
    return SHLCMD_HIST | SHLCMD_DONE;
}
...
static int _stopCmd(int argc, char **argv)
{
    ...
    printf("PWM stopped on channel mask: 0x%lx\n", channel_mask);
    return SHLCMD_HIST | SHLCMD_DONE;
}
...
static int _demoCmd(int argc, char **argv)
{
    ...
    WISE_PWM_CONF_T pwm_conf = {
        .common.mode            = PWM_MODE_FREQUENCY,
        .u.freq.frequency_Hz    = 100,
        .u.freq.duty_percent    = 75,
    ...
    if (wise_pwm_configure(pwm_channel, gpio_pin, &pwm_conf) == WISE_SUCCESS) {
        printf("PWM channel %d configured with frequency %ldHz duty percent "
               "%ld and interrupt enabled\n",
               pwm_channel, pwm_conf.u.freq.frequency_Hz, pwm_conf.u.freq.duty_percent);
        wise_pwm_register_callback(pwm_channel, my_pwm_callback, NULL);
    } else {
        printf("Failed to configure PWM channel\n");
        return SHLCMD_HIST | SHLCMD_DONE;
    }
    printf("PWM started channel %d on gpio%d \n", pwm_channel, gpio_pin);
    wise_pwm_start(1UL << pwm_channel);
